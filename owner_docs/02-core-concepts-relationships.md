...
// TaskInput - Shannon任务的"宪法"
// 这个结构体定义了AI系统理解和执行用户意图的完整框架
type TaskInput struct {
    // ========== 核心身份标识 ==========
    // 这四个字段构成了任务的"身份证"
    Query     string `json:"query"`     // 用户原始查询 - 必填，不能为空
    UserID    string `json:"user_id"`    // 用户标识 - 多租户和个性化基础
    TenantID  string `json:"tenant_id"`  // 租户标识 - 数据隔离的关键
    SessionID string `json:"session_id"` // 会话标识 - 连续性保证的核心

    // ========== 执行环境增强 ==========
    Context   map[string]interface{} `json:"context,omitempty"` // 扩展上下文 - 运行时参数传递

    // ========== 执行策略控制 ==========
    Mode      string `json:"mode,omitempty"` // 执行模式: simple/complex/research
    // 为什么是字符串而不是枚举？扩展性！

    // ========== 模板系统集成 ==========
    TemplateName    string `json:"template_name,omitempty"`    // 模板名称 - 声明式配置
    TemplateVersion string `json:"template_version,omitempty"` // 模板版本 - 版本控制
    DisableAI       bool   `json:"disable_ai,omitempty"`       // 禁用AI - 纯模板模式

    // ========== 会话连续性 ==========
    History    []Message              `json:"history,omitempty"`    // 最近对话历史
    SessionCtx map[string]interface{} `json:"session_ctx,omitempty"` // 持久化会话状态

    // ========== 人工干预机制 ==========
    RequireApproval bool `json:"require_approval,omitempty"` // 是否需要人工批准
    ApprovalTimeout int  `json:"approval_timeout,omitempty"` // 批准超时(秒)

    // ========== 性能优化 ==========
    BypassSingleResult bool `json:"bypass_single_result,omitempty"` // 单结果直接返回

    // ========== 嵌套执行支持 ==========
    ParentWorkflowID string `json:"parent_workflow_id,omitempty"` // 父工作流ID

    // ========== 预分解信息 ==========
    SuggestedTools   []string               `json:"suggested_tools,omitempty"`   // 建议工具
    ToolParameters   map[string]interface{} `json:"tool_parameters,omitempty"` // 预结构化参数

    // ========== 资源控制 ==========
    MaxTokens      int     `json:"max_tokens,omitempty"`      // 最大令牌数
    BudgetLimitUSD float64 `json:"budget_limit_usd,omitempty"` // 预算上限(美元)

    // ========== 质量保证 ==========
    MinConfidence float64 `json:"min_confidence,omitempty"` // 最小置信度
    RequireCitations bool  `json:"require_citations,omitempty"` // 需要引用

    // ========== 执行约束 ==========
    TimeoutSeconds int `json:"timeout_seconds,omitempty"` // 总超时时间
    Priority       int `json:"priority,omitempty"`        // 执行优先级(1-10)

    // ========== 可观测性 ==========
    DebugMode      bool   `json:"debug_mode,omitempty"`      // 调试模式
    RequestID      string `json:"request_id,omitempty"`      // 请求追踪ID
    UserAgent      string `json:"user_agent,omitempty"`      // 客户端信息
